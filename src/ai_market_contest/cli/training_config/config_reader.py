...
class TrainingConfigReader:
    ...
    def get_naive_agent_counts(self) -> Dict[str, int]:
        return {
            ...
            for agent_name, agent_count in self.parsed_config["Naive Agents"]
        }

    def get_num_agents(self) -> int:
        return self.get_self_play_num + sum(self.get_naive_agent_counts().items())

    def get_other_config(self) -> Dict[str, Any]:
        return {
            (setting_name, literal_eval(setting_value))
            for setting_name, setting_value in self.parsed_config["Other"]
        }
