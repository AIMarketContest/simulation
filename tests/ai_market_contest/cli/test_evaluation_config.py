...
from ai_market_contest.cli.configs.evaluation_config_reader import EvaluationConfigReader
from ai_market_contest.training.policy_config_maker import PolicyConfigMaker
from ai_market_contest.cli.configs.simulation_config_reader import SimulationConfigReader
...
def test_training_config(tmp_path):
    ...
    evaluation_config_reader = EvaluationConfigReader(tmp_path/"evaluation_configs/evaluation_example_config.ini", demand_function_locator, config_parser, 0)

    assert evaluation_config_reader.get_naive_agent_counts() == {'FixedFifty': 3, 'Random': 1}
