...
from ai_market_contest.cli.adddemandfunctionsubcommand import edit_environment_config_file, remove_demand_function, create_demand_function
...
def test_create_demand_function(tmp_path):
    ...
    demand_function_file = tmp_path / "environments" / "demandfunctions" / "test_demand_function.py"
    env_config_file = tmp_path / "environments" / "config.ini"

    assert demand_function_file.exists()
    assert "class test_demand_function(metaclass=ABCMeta):" in demand_function_file.read_text()
