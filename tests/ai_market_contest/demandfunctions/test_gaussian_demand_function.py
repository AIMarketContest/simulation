...
class GaussianDemandFunctionTest(TestCase):
    ...
    def test_function_returns_correct_number_of_demands(self):
        sales_with_four_prices = self.demand_function_with_default_sf.get_sales(
            {"agent_0": 10, "agent_1": 15, "agent_2": 10, "agent_3": 8}
        )
        assert len(sales_with_four_prices) == 4
        sales_with_three_prices = self.demand_function_with_default_sf.get_sales(
    ...
    def test_function_returns_gaussian_(self):
        assert (
            self.demand_function_with_default_sf.get_sales({"agent_0": 50})["agent_0"]
            == 100
        )

        assert (
            self.demand_function_with_sf_ten.get_sales({"agent_0": 30})["agent_0"]
            == 100
        )
